...
# Modified By tanqiang(tq17bio@qq.com)
# @modified 2025-02-10
...
import pysam
import numpy
from structures import SAMRead, MSILocus
...
class MSILocusLoader:
    def __init__(self, config):
        ...
        if "genome" in config:
            ...
            tprint(
                "MSILocusLoader warning: You need to specify a path"
    ...
    def load_loci(self, bedfile):
        ...
        if not os.path.isfile(bedfile):
            tprint(
                "MSILocusLoader error: File {0}".format(bedfile) + " does not exist!"
            )
        else:
            with open(bedfile, "r") as filein:
                for line in filein.readlines():
                    if line[0] != "@":
                        locus = MSILocus(line)
                        # if locus.chromosome[0:3] != 'chr': #needn't add chr prefix, modified by tanqiang
    ...
    def correct_off_by_one_errors(self, locus):
        ...
        position = "{0}:{1}-{2}".format(
            locus.chromosome, locus.start - 1, locus.end + 1
        )
        ...
        if sequence[1 : 1 + locus.kmer_length] != locus.kmer:
            ...
            if sequence[0 : locus.kmer_length] == locus.kmer:
                ...
                tprint(
                    "Error: Specified locus does not appear "
...
class KmerRepeatCounter:
    ...
    def read_has_truncation(self, read, locus, offset, repeat_count):
        truncated = False
        if (
            read.length - (offset + (repeat_count * locus.kmer_length))
            < locus.kmer_length
        ):
    ...
    def passes_locus_qc_filter(self, read, offset_start, offset_end):
        locus_mean_quality = KmerRepeatCounter.subset_mean_quality(
            read, offset_start, offset_end
        )
    ...
    def locus_repeat_count(self, read, locus):
        ...
        if read.start <= locus.start and read.end >= locus.end:
            ...
            if offset < read.length:
                ...
                repeat_count = KmerRepeatCounter.kmer_repeat_count(
                    locus.kmer, read.seq, offset
                )
    ...
    def read_analyzer(
        self, queue_in, queue_out, full, empty, mutex_in, mutex_out, queue_full
    ):
        ...
        while True:
            ...
            if self.debug_output and (n % 10000 == 0):
                tprint(
                    "Analyzer> Thread {0}".format(os.getpid())
            ...
            if self.passes_qc_filter(read):
                ...
                if repeat_count >= 0:
                    ...
                    if self.passes_locus_qc_filter(read, repeat_start, repeat_end):
                        ...
                        queue_full.acquire()
                        mutex_out.acquire()
                        queue_out.put([locus.name, repeat_count])
                        mutex_out.release()
    ...
    def status_check(self, qsize):
        ...
        if self.has_live_producer() and not self.has_live_consumers():
            tprint(
                fail
                + "Main> Analyzer(s) LIVE, Extractor DEAD, Queue "
                + queue_status
                + ". "
                + reset
            )
            tprint(
                fail
                + "Main> Teriminating process due to "
    ...
    def process(self, input_filepath, msi_loci, config):
        ...
        if self.debug_output:
            tprint(
                "Main> Generating {0} analyzer process(es).".format(consumer_threads)
            )
        for i in range(0, consumer_threads):
            p = Process(
                target=self.read_analyzer,
                args=(
                    queue_in,
                    queue_out,
                    full,
...
def generate_config(args):
    ...
    if args.normal is None:
        tprint("Error: Normal BAM/SAM file(s) not provided!")
        exit(1)
    else:
        # 遍历所有提供的文件路径
        config["normal_filepaths"] = []
        for filepath in args.normal:
            abs_filepath = os.path.abspath(filepath)  # 获取绝对路径
            if not os.path.isfile(abs_filepath):
                tprint(f"Error: {abs_filepath} does not exist!")
                exit(1)
            config["normal_filepaths"].append(abs_filepath)
    ...
    if args.output is None:
        ...
        config["output_norm_dir"] = (
            os.path.dirname(config["output_filepath"]) + "/norms"
        )
        if not os.path.isdir(config["output_norm_dir"]):
            os.makedirs(config["output_norm_dir"])
...
def normalize_dataframe(df):
    ...
    for column in df.columns:
        ...
        if column_total != 0:
            df[column] = (df[column] / column_total) * 100
    return df.round(2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",  # 允许接受多个值
        required=True,
        help="Normal input (SAM/BAM) file(s). Multiple files can be provided.",
    )
    ...
    parser.add_argument(
        "--genome", dest="genome", type=str, help="Path to reference genome (FASTA)."
    )
    ...
    tprint("Processing normal input files , save to {}".format(config["output_norm_dir"]))
    for normal_filepath in config["normal_filepaths"]:
        normal_out_fname = f"{normal_filepath.split('/')[-1].split('.')[0]}.tsv"
        generate_index_if_needed(normal_filepath)
        normal = krc.process(normal_filepath, msi_loci, config)
        ...
        if combined_df.empty:
            ...
            combined_df = combined_df.add(df_fill, fill_value=0)
    combined_df = normalize_dataframe(combined_df)
    tprint("baseline output file , save to {}".format(config["output_filepath"]))
    combined_df.to_csv(config["output_filepath"], sep="\t")

    tprint("Done processing baseline.")
