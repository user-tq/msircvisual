...
# raw ulr of the file: https://github.com/esko/mantis/blob/master/structures.py

# Modified By tanqiang(tq17bio@qq.com)
# @modified 2025-02-07
# you can search "modified by tanqiang" to find all modifications
...
class Locus(object):
    ...
    def locus(self):
        locus = "{0}:{1}-{2}".format(self.chromosome, self.start, self.end)
        return locus
        # end .locus()
    ...
    def __str__(self):
        out = "{0} covering {1}".format(self.__class__.__name__, self.locus())
        return out
        # end .__str__()
...
# modified by tanqiang
# extract the bed columns4
...
class MSILocus(Locus):
    def __init__(self, line):
        ...
        # modified by tanqiang
        # for new bed format
        self.kmer, self.repeats, self.name = extract_content(line[3])
        if self.kmer is None:
            # error
            print("Error: bed file format error, please check the bed file format")
            exit(1)
        self.kmer_length = len(self.kmer)
        # self.strand = line[5]
        # print(self.kmer, self.kmer_length, self.repeats, self.name, self.strand)
        # end .__init__()
